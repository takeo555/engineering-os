...
**このファイルが日次記録のフォーマット正本。** ChatGPTはレビューの最後にこの形で1つのコードブロックをチャットへ出す。独自フォーマットは作らない。
ユーザーはそれをコピーし、GitHubの **Issue #1「📥 記録の投函口」へコメントとして貼る。** 記録のために新しいIssueは作らない。

## 崩してはいけない点

- ヘッダは `---` で挟む。`key: value` の1行1項目のみ。入れ子・配列・引用符・コメントを書かない
- **ここに無いヘッダキーを足さない。** ヒント使用などの情報は `## 3. Review` に書く
...
- 5軸はレビュー時に確定した点数をそのまま写す。記録ブロック作成時に再計算しない
