...
Claude Pro の定期タスクは時刻を指定できる。GitHub コネクタ（MCP）を ON にしておく。
...
GitHub コネクタで、既存の Issue #1「📥 記録の投函口」に **コメント** する。
コメント本文は、上の記録ブロックそのもの。
新しい Issue は作らない。

投稿できたあと、チャットには1行だけ添える。

> 投函口に投稿しました。Bank の adopt は自分で書き写してください。
