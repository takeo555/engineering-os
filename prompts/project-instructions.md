...
回答を受けたら正誤判定し、**1行だけフィードバック**して、そのまま Design に進みます。ここで長い解説をしません（解説は記録ブロックで行います）。
...
Drill の正誤判定結果は、Design 出題前にユーザーに **簡潔に1行フィードバック** する（詳細解説は Design 終了後の記録ブロック内で行う）。

例:
> Drill: AI 3/3, 言語 2/3, ネットワーク 1/2。間違い2つは Design 終了後に解説します。続けて Design 15分問題に進みます。
