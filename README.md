...
- **読み取り:** 試験官が `TODAY.md` のraw URLを開く（落ちたら jsDelivr → 貼付依頼 → おまかせ出題）
- **書き込み:** 投函口のコメントを、Workflowがパースしてコミットする

投函を自動化する検討の経緯は [connector.md](10-automation/connector.md)（**現時点では未導入**）。
...
| 記録ブロックを投函口へ貼る（毎回30秒） | 記録ブロックをチャットに出す |
| 週1回、Bank昇格候補を書き写す（3分） | Weekly Reviewの講評を書く |
| 週1回20分、実機で確認する | — |
| 機密情報を除いて共有する | — |

手動で残るのは3つです。**20分解いて答えを書く / 記録ブロックを貼る / 週1回Bank昇格を書き写す。** Bankへの書き込みを自動化していないのは、自動で流し込むとBankが使われない要約の山になるからです。
