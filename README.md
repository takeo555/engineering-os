...
60秒サマリ → 詳細レビュー（正誤 / 良かった点 / 不足 / 実務ならどう考えるか / 点数）
    ↓
試験官が記録ブロックを GitHub の「記録の投函口」へ投稿する
    ↓
...
毎朝7時にリマインドが届きます。Projectで「今日の1問」と送って目安30分で解き、回答を貼るとレビューと採点が返ります。試験官が記録ブロックを GitHub の Issue #1「📥 記録の投函口」へ投稿すれば、あとは自動です。新しいIssueは作りません。

```text
06:00 GitHub Actions（build-context）：弱点と履歴から、今日のTrack / Format / Level / 狙う弱点を決めて
      CONTEXT.md に書く（問題文は作らない）
  ↓
07:00 Claudeの定期タスク：「Projectを開いて『今日の1問』と送って」と通知するだけ（出題しない）
  ↓
自分：Project「Engineering OS 試験官」を開いて「今日の1問」と送る
  ↓
Claude：CONTEXT.md を読み、その割り当て通りに1問出す（評価観点はこの時点で固定）
  ↓
自分：目安30分で解いて、同じ会話に回答を貼る
  ↓
Claude：必要ならヒント（答えは出さない）→ 60秒サマリ → 詳細レビュー → 採点確定
      → 記録ブロックをチャットへ出力 → 投函口へコメント
  ↓
...
採点が終わると、試験官が **記録ブロック** をチャットに出します。それを **自分で** Issue #1「📥 記録の投函口」へコメントとして貼ります（30秒）。**新しいIssueは作りません。** claude.ai の GitHub 連携は読み取り専用なので、試験官は投稿できません。

- **読み取り:** 試験官が `CONTEXT.md` のraw URLを開く
- **書き込み:** 投函口のコメントを、Workflowがパースしてコミットする
...
| 記録ブロックを投函口へ貼る（毎回） | 記録ブロックをチャットに出す |
| 週1回、Bank昇格候補を書き写す（3分） | Weekly Reviewの講評を書いて投稿する |
| 週1回20分、実機で確認する | — |
| 機密情報を除いて共有する | — |

手動で残るのは3つです。**「今日の1問」と送る / 30分解いて回答を貼る / 週1回Bank昇格を書き写す。** Bankへの書き込みを自動化していないのは、自動で流し込むとBankが使われない要約の山になるからです。
...
| [`CONTEXT.md`](CONTEXT.md) | 自動生成。**試験官が毎朝読む唯一の入り口**。今日の割り当てと弱点と履歴 |
