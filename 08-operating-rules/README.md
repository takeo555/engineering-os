...
- 記録ブロックを Issue #1「📥 記録の投函口」へ自分でコメントする（試験官は投稿できない）
- 朝は30秒サマリだけ読めばよい。詳細レビューは夜に記録ファイルで読んでよい
...
3. ユーザーが回答する
4. 必要に応じてヒント（答えは直接出さない。評価基準は変えない）
5. 回答レビュー
6. 採点を一度だけ確定する（チャット上の点と記録の点は同じにする）
7. 自力到達 / ヒント後到達 / 未到達を整理する
8. weakness / 次回課題を決める
9. [`prompts/record-block.md`](../prompts/record-block.md) に従って記録ブロックを作る
10. チャット上に記録ブロックを出す
11. 記録ブロックを既存の Issue #1「📥 記録の投函口」へ自分でコメントする

守ること:

- ヒントで補った重要な観点は、最終回答として正しくても弱点・再テストに残す。ヒント利用そのものだけを理由に減点し直さない
- 記録ブロック作成時に点数を再計算しない
- 制約時間と実測時間を混同しない。実測が不明なら推測しない
- 記録フォーマットは `record-block.md` が正本。ヘッダキーを勝手に足さない
- 学習記録のために新しいIssueを作らない
