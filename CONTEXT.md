...
|---|---|---|---|
| AI | 3 | 法制度と倫理（EU AI Act, 日本の指針、ガバナンス）；リスク（幻覚、著作権、秘密漏洩、バイアス）；生成AIの仕組み概要（Transformer, LLM, RAG, ファインチューニング） | なし |
| 言語 | 3 | Next.js / fetch のキャッシュ；Terraform / workspace；Next.js / Server Actions | なし |
| ネットワーク | 2 | 2章 プロトコル・スタックとLANアダプタ；5章 ファイアウォール・キャッシュ・負荷分散 | なし |
...
- Level: 4日空いたが L1 が最下位のため据え置き
- Format: モバイル想定のためDebugを選択
...
<!-- eos:context date=2026-09-15 track=Layered Architecture format=Debug level=L1
     targets=W005
