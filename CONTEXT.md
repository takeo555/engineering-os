...
|---|---|---|---|
| AI | 3 | 業務利用の注意点；リスク（幻覚、著作権、秘密漏洩、バイアス）；生成AIの仕組み概要（Transformer, LLM, RAG, ファインチューニング） | なし |
| 言語 | 3 | Python / async/await；Terraform / state と backend；Python / generator | なし |
| ネットワーク | 2 | 6章 Webサーバ側の処理；1章 Webブラウザがメッセージを作る | なし |
...
<!-- eos:context date=2026-09-18 track=DB / Table Design format=Explain level=L1
     targets=W001
